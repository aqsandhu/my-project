...
export default async function Page({
	...
	async function addItem(formData: FormData) {
		...
		const checkoutId = await Checkout.getIdFromCookies("default-channel");
		...
		try {
			const response = await fetch(
				new URL("/api/checkout/lines/add", process.env.NEXT_PUBLIC_STOREFRONT_URL).toString(),
				{
					method: "POST",
					headers: {
			...
			if (!response.ok) {
				...
				return {
					message: errorResponse.message || "Error adding item to cart",
					status: "error",
				};
			}
