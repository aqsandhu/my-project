...
# Use try-except for Sentry SDK imports to make them optional
try:
    import sentry_sdk
    ...
    # Define empty constants when Sentry SDK is not available
    DEFAULT_DENYLIST = []
    DEFAULT_PII_DENYLIST = []

# Try importing jaeger_client with fallback
try:
    import jaeger_client.config
except ImportError:
    warnings.warn(
        "Jaeger client is not installed. Tracing will not be available. "
        "To enable, install jaeger-client package.",
        ImportWarning,
    )
...
from graphql.execution import executor as graphql_executor
from pytimeparse import parse

# TODO: Remove these imports since they are now in the try block below
...
# Find MIDDLEWARE list and add our security middleware
MIDDLEWARE = [
    # Add SecurityMiddleware at the beginning of the list
    "saleor.core.middleware.SecurityMiddleware",
    "saleor.core.security_headers.SecurityHeadersMiddleware",
...
# Find SECURE_PROXY_SSL_HEADER and update security settings
...
CSP_SCRIPT_SRC = ("'self'", "'unsafe-inline'", "'unsafe-eval'", "https://cdn.yourdomain.com")
CSP_STYLE_SRC = ("'self'", "'unsafe-inline'", "https://cdn.yourdomain.com")
CSP_IMG_SRC = ("'self'", "data:", "https:", "http:")
CSP_FONT_SRC = ("'self'", "https://cdn.yourdomain.com", "data:")
CSP_CONNECT_SRC = ("'self'", "https://api.yourdomain.com")

# Security auditing
ENABLE_AUDIT_LOGS = True

# Enhanced logging configuration for security
LOGGING = {
    "version": 1,
    ...
    "handlers": {
        ...
        "file": {
            ...
            "filename": os.path.join(PROJECT_ROOT, "logs", "saleor.log"),
        ...
        "security_file": {
            "level": "INFO",
            "class": "logging.handlers.RotatingFileHandler",
            "filename": os.path.join(PROJECT_ROOT, "logs", "security.log"),
            "maxBytes": 10 * 1024 * 1024,  # 10 MB
            "backupCount": 10,
            "formatter": "json",
        },
...
# Set up Sentry options only if SDK is available
if SENTRY_SDK_AVAILABLE:
    try:
        ...
        SENTRY_SDK_AVAILABLE = False
        warnings.warn(
            f"Sentry SDK options error: {e}. Error tracking is disabled.",
            ImportWarning,
        )
        SENTRY_OPTS = {}
else:
...
try:
    ...
    warnings.warn(
        f"Failed to patch SubscriberExecutionContext: {e}",
        RuntimeWarning,
    )
...
def SENTRY_INIT(dsn: str, sentry_opts: dict):
    ...
    """
    # If Sentry SDK is not available, return early to avoid errors
    if not SENTRY_SDK_AVAILABLE:
        warnings.warn(
            "Sentry SDK is not installed or properly configured. Error tracking is disabled. "
            "To enable, install sentry-sdk package.",
            ImportWarning,
        )
        return
    ...
    try:
        ...
        warnings.warn(
            f"Sentry SDK initialization failed: {e}. Error tracking is disabled.",
            ImportWarning,
        )
        return
